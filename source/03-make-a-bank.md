...
Open the folder `db/migrations`, you should see two files:
...
```ruby
:::-> $ ls -Ad1 db/migrate/* | grep create_accounts.rb | xargs cat | grep users
```
...
```
> user = User.first
:::-> $ echo "user = User.first" | bin/rails console
```
...
:::-> echo  "Account.create(balance: 50.0, user_id: User.first.id)" | bin/rails c
```
...
> puts account.user
:::-> $ echo "Account.first.user" | bin/rails c
```
...
:::-> $ echo "User.first.accounts.first" | bin/rails c
```
...
First we need to modify our migration open up your migration file in `db/migrations` that ends with `_create_transactions.rb`.
...
```
:::>> file.remove db/migrations/*_create_transactions.rb
...
```
:::>> file.append db/migrations/*_create_transactions.rb#4
...
```
:::>> file.append app/models/transaction#2
...
:::-> echo "Account.create(balance: 50, user_id: 1); Account.create(balance: 50, user_id: 2)" | rails c
```
...
If you try to enter some account IDs and an amount it will work, but nothing will be transfered, yikes. Why not? Rails has no idea that we intended that behavior, we'll have to manually program that logic. Don't worry though, it's pretty short.
